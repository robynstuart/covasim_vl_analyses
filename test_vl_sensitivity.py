...
def test_vl_testing(fig_path=None):
    ...
    tn3 = cv.test_num(daily_tests=n_tests, lod=3.0, symp_test=1.0, start_day=start_day, test_delay=delay)
    tn5 = cv.test_num(daily_tests=n_tests, lod=5.0, symp_test=1.0, start_day=start_day, test_delay=delay)
    ...
    sim3n = cv.Sim(pars, interventions=tn3, label='PCR tests, test_num')
    sim5n = cv.Sim(pars, interventions=tn5, label='Rapid tests, test_num')
    sim3p = cv.Sim(pars, interventions=tp3, label='PCR tests, test_prob')
    sim5p = cv.Sim(pars, interventions=tp5, label='Rapid tests, test_prob')

    # Run and plot results
    to_plot = ['new_infections', 'cum_infections', 'cum_inf_neg', 'cum_noninf_pos', 'new_diagnoses', 'cum_diagnoses', 'new_tests', 'test_yield']
...
if __name__ == '__main__':
    ...
    sc.tic()
#    y = sim_vls(n=100, n_days=28)
#    msim_vls(n=100)
    msim = test_vl_testing()

#    method = 'new'
#    sim = test_vl_sensitivity(beta=0.003, method=method, do_plot=do_plot, do_save=do_save)
#    font_size = 12
#    pl.rcParams['font.size'] = font_size
#    sim.plot(do_save=True, do_show=False, fig_path=f'results/plot_sim_{method}.png')
    sc.toc()
